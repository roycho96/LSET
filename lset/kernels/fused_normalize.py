"""Fused L2 Normalize — drop-in replacement for F.normalize(x, p=2, dim=-1).

Row-wise L2 normalization in a single Triton kernel, halving global memory reads.

Standard F.normalize:
  Kernel 1: norm = sqrt(sum(x^2, dim=1))   <- N*D read
  Kernel 2: out = x / norm                 <- N*D read + N*D write

Fused:
  Kernel 1: read x -> sum_sq -> norm -> x/norm -> write   <- N*D read + N*D write (1-pass)

Backward:
  y = x / ||x||
  dx = (grad_y - y * <y, grad_y>) / ||x||

Ported from ~/workspace/dense/optimization/kernels/embedding/fused_normalize.py.
"""
