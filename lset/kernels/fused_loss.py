"""
Ported from ~/workspace/dense/optimization/kernels/embedding/fused_loss.py

Fused Dense Embedding Loss (Triton + Python Hybrid)

Computes contrastive loss without storing the score matrix by recomputing Q@K.T
tile-by-tile during forward/backward, saving memory.

Supported losses:
  - "multi" (MP-NCE): Multi-Positive NCE Loss
  - "soft"  (Soft CE): Soft Label Cross Entropy Loss
  - "cross" (Cross CE): Standard Cross Entropy Loss

Architecture:
  - 1 Forward Triton kernel: _lse_fwd_kernel (LSE_MODE constexpr branches neg/all)
  - 2 Backward Triton kernels: dQ, dK (LOSS_TYPE constexpr branching)
  - Python wrapper: per-loss-type forward computation + backward preparation

Mathematical structure of the 3 loss types:
  MP-NCE:
    Expensive op: neg_lse = logsumexp(negative scores)  <- neg_lse kernel
    Cheap op: pos_scores = q[qi] . k[di]                <- O(P*D) Python
    loss = softplus(neg_lse - pos_score)
  Soft/Cross CE:
    Expensive op: all_lse = logsumexp(valid scores)      <- all_lse kernel
    Cheap op: pos_scores = q[qi] . k[di]                <- O(P*D) Python
    soft:   loss = all_lse - sum(norm_label * pos_score)
    cross:  loss = label_sum * all_lse - sum(label * pos_score)

Backward grad_s (compile-time branching in kernel, zero runtime cost):
  multi:  pos -> w*(sigmoid(s-neg_lse)-1),  neg -> w*exp(s-neg_lse)*sum_sigma
  soft:   valid -> w*(softmax(s, all_lse) - norm_label)
  cross:  valid -> w*(label_sum*softmax(s, all_lse) - label)

Requirements:
  - int8 labels (-1=ignore, 0=neg, >0=pos). float32+NaN is handled by
    fallback on the caller side (loss.py).
  - Triton >= 2.1

Key optimizations (cumulative v1~v6):
  A. NaN guard: handle -inf max in online logsumexp
  B. Scale absorption: q_scaled = q * scale
  C. Safe exp: zero out backward when neg_lse=-inf for a query
  D. bf16 tensorcore: preserve input dtype, tl.dot uses native precision
  E. Autotune + BLOCK_D expansion: search up to BLOCK_D=256
  F. GROUP_N/M heuristic: tile grouping to reduce reload count
  G. Branchless forward: prevent GPU-CPU synchronization
  H. dQ BM=16 config: improve SM occupancy
  I. should_use_fused(): hierarchical threshold based on Q*K score matrix + Q (v6.1)
  J. pos_qi/pos_di external input: eliminate Q*K scan
  K. neg_mask removal: use neg_counts for valid determination
  L. softplus transform: reduce kernel launches
  M. pos_scores float conversion removal: bf16 multiply + fp32 accumulate
  N. dQ num_stages=1: reduce register pressure
  O. pos_scores no_grad+detach: prevent autograd graph creation outside Function
  P. LSE kernel unification: neg_lse + all_lse -> 1 kernel + LSE_MODE constexpr
  Q. CE backward valid_mask: pos_mask|neg_mask -> (labels>=0), reduce registers
  R. CE forward NaN guard: prevent 0*(-inf)=NaN on invalid queries
  S. q_bucket autotune: separate autotune configs by Q size (fwd, dQ) (v6.1)
  T. fwd BM=16 config: improve SM occupancy for small Q (v6.1)
  U. dQ BM=8: not possible due to tl.dot M>=16 constraint. dQ minimum BM=16.
"""
