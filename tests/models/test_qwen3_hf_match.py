...
def test_hf_match():
    ...
    assert lset_hidden.shape == hf_hidden.shape, f"Shape mismatch: LSET {lset_hidden.shape} vs HF {hf_hidden.shape}"

    # Compare
    max_diff = (lset_hidden - hf_hidden).abs().max().item()
    print(f"Max absolute difference: {max_diff:.6e}")
    assert torch.allclose(lset_hidden, hf_hidden, atol=1e-4), f"Hidden states differ. Max diff: {max_diff:.6e}"
