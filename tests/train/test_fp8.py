...
class TestFP8Training:
    ...
    def test_fp8_training_steps_with_compile(self, fp8_model):
        ...
        assert all(not torch.isnan(torch.tensor(v)) for v in losses), f"NaN: {losses}"
    ...
    def test_fp8_memory_vs_bf16(self):
        ...
        assert fp8_mem < bf16_mem * 2.0, f"FP8 mem {fp8_mem / 1e6:.1f}MB >> BF16 mem {bf16_mem / 1e6:.1f}MB"
